import CoreGraphics

enum NotchContentTopAccessoryHorizontalAlignment: Equatable {
    case leading
}

enum NotchExpandedLayout {
    static let topPadding: CGFloat = 4
    static let bottomPadding: CGFloat = 10
    static let headerHeight: CGFloat = 32
    static let headerContentSpacing: CGFloat = 8
    static let contentTopAccessoryHeight: CGFloat = headerHeight
    static let contentTopAccessoryHorizontalAlignment: NotchContentTopAccessoryHorizontalAlignment = .leading
    static let safeHorizontalPadding: CGFloat = 27
    static let pluginTabSize = CGSize(width: 34, height: 24)
    static let pluginTabIconSize: CGFloat = 13
    static let settingsButtonSize: CGFloat = 24
    static let settingsIconSize: CGFloat = 10
    static let globalDropStripHeight: CGFloat = 38

    static func pluginViewportHeight(
        forDisplayHeight displayHeight: CGFloat,
        includesContentTopAccessory: Bool = false
    ) -> CGFloat {
        let reservedContentTopAccessoryHeight = includesContentTopAccessory
            ? contentTopAccessoryHeight + headerContentSpacing
            : 0

        return max(
            0,
            displayHeight
                - topPadding
                - headerHeight
                - headerContentSpacing
                - reservedContentTopAccessoryHeight
                - bottomPadding
        )
    }

    static func dropStripHeight(for state: NotchGlobalDropStripState) -> CGFloat {
        state.isVisible ? globalDropStripHeight : 0
    }
}
