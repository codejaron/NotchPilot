...
private struct AIPluginExpandedView<Plugin: AIPluginRendering>: View {
    ...
    private func attentionAccent(hasAttention: Bool, accent: Color) -> some View {
        Capsule()
            .fill(hasAttention ? accent : Color.clear)
            .frame(width: 2, height: 22)
    }

    @ViewBuilder
    private func sessionMetaColumn(_ summary: AIPluginExpandedSessionSummary) -> some View {
        VStack(alignment: .trailing, spacing: 2) {
            if summary.hasTokenUsage {
                HStack(spacing: 6) {
                    Text("↑\(plugin.formattedTokenCount(summary.inputTokenCount))")
                    Text("↓\(plugin.formattedTokenCount(summary.outputTokenCount))")
                }
                .font(.system(size: 10, weight: .medium, design: .rounded))
                .monospacedDigit()
                .foregroundStyle(NotchPilotTheme.islandTextSecondary.opacity(0.82))
    ...
    private func claudeQuestionCard(_ approval: PendingApproval) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            ForEach(approval.payload.claudeQuestions) { question in
                claudeQuestionBlock(question, approval: approval)
            }

            HStack(spacing: 8) {
                Button {
                    submitClaudeQuestionSkip(for: approval)
                } label: {
                ...
                Button {
                    submitClaudeQuestionAnswers(for: approval)
                } label: {
                    Text(AppStrings.text(.submit, language: settingsStore.interfaceLanguage))
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: 8, style: .continuous)
                                .fill(
                                    canSubmitClaudeQuestionAnswers(for: approval)
                ...
                .buttonStyle(.plain)
                .disabled(canSubmitClaudeQuestionAnswers(for: approval) == false)
            }
        }
        .onAppear {
            ensureClaudeQuestionDefaults(for: approval)
        }
    }

    private func claudeQuestionBlock(
        _ question: ClaudeUserQuestion,
        approval: PendingApproval
    ) -> some View {
        let key = claudeQuestionKey(approval: approval, question: question)
        let showsTextInput = question.options.isEmpty
            || selectedClaudeQuestionLabels(for: approval, question: question).contains(where: isFreeformClaudeQuestionOption)
        ...
        return VStack(alignment: .leading, spacing: 8) {
            ...
            if question.options.isEmpty == false {
                VStack(alignment: .leading, spacing: 6) {
                    ForEach(question.options) { option in
                        claudeQuestionOptionRow(option, question: question, approval: approval)
                    }
                }
            }
            ...
            if showsTextInput {
                TextField(
                    ...
                    text: Binding(
                        get: {
                            claudeQuestionTextAnswers[key, default: ""]
                        },
                        set: { value in
                            claudeQuestionTextAnswers[key] = value
                        }
                    )
                )
    ...
    private func claudeQuestionOptionRow(
        _ option: ClaudeQuestionOption,
        question: ClaudeUserQuestion,
        approval: PendingApproval
    ) -> some View {
        let isSelected = selectedClaudeQuestionLabels(for: approval, question: question).contains(option.label)

        return Button {
            toggleClaudeQuestionOption(option.label, question: question, approval: approval)
        } label: {
    ...
    private func ensureClaudeQuestionDefaults(for approval: PendingApproval) {
        for question in approval.payload.claudeQuestions {
            let key = claudeQuestionKey(approval: approval, question: question)
            if claudeQuestionSelections[key] == nil,
               let firstOption = question.options.first {
                claudeQuestionSelections[key] = [firstOption.label]
            }
        }
    }

    private func toggleClaudeQuestionOption(
        _ label: String,
        question: ClaudeUserQuestion,
        approval: PendingApproval
    ) {
        let key = claudeQuestionKey(approval: approval, question: question)
        var selections = claudeQuestionSelections[key] ?? []
        ...
        claudeQuestionSelections[key] = selections
    }

    private func selectedClaudeQuestionLabels(
        for approval: PendingApproval,
        question: ClaudeUserQuestion
    ) -> Set<String> {
        claudeQuestionSelections[claudeQuestionKey(approval: approval, question: question)] ?? []
    }

    private func claudeQuestionKey(approval: PendingApproval, question: ClaudeUserQuestion) -> String {
        "\(approval.requestID)::\(question.id)"
    }

    private func isFreeformClaudeQuestionOption(_ label: String) -> Bool {
        let normalized = label.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        return normalized == "other" || normalized == "其他"
    }

    private func claudeQuestionAnswers(for approval: PendingApproval) -> [String: String] {
        approval.payload.claudeQuestions.reduce(into: [:]) { answers, question in
            let key = claudeQuestionKey(approval: approval, question: question)
            let selected = selectedClaudeQuestionLabels(for: approval, question: question)
            let selectedLabels = question.options.map(\.label).filter { selected.contains($0) }
            let textAnswer = claudeQuestionTextAnswers[key]?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
            let answerParts = selectedLabels.compactMap { label -> String? in
                if isFreeformClaudeQuestionOption(label), textAnswer.isEmpty == false {
                    return textAnswer
                }
                return label
            }
    ...
    private func canSubmitClaudeQuestionAnswers(for approval: PendingApproval) -> Bool {
        claudeQuestionAnswers(for: approval).count == approval.payload.claudeQuestions.count
    }

    private func submitClaudeQuestionAnswers(for approval: PendingApproval) {
        let answers = claudeQuestionAnswers(for: approval)
    ...
    private func submitClaudeQuestionSkip(for approval: PendingApproval) {
        let action = ApprovalAction(
    ...
    private func approvalCard(_ approval: PendingApproval) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            codexApprovalSummary(claudeApprovalSummary(for: approval))
